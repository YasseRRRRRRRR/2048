...
def drawBoard(screen, board):
    ...
    fontImage = scoreFont.render("Score: {:,}".format(board.score) + (" [AI enabled, depth={}]".format(depth) if Agent else ""), 1, white)
...
def handleInput(event, board):
    ...
    if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_RIGHT:
            board.move(alternateLogic.RIGHT)
        elif event.key == pygame.K_LEFT:
            board.move(alternateLogic.LEFT)
        elif event.key == pygame.K_UP:
            board.move(alternateLogic.UP)
        elif event.key == pygame.K_DOWN:
            board.move(alternateLogic.DOWN)
        if event.key == pygame.K_r:
            board = alternateLogic.Board(boardSize)
...
def gameLoop():
    ...
    while 1:
        ...
        if Agent and not board.checkLoss():
